...
class Player:
    ...
    def showProperties(self):
        ...
        for prop in self.properties:
            name = prop.properties["name"] + " (" + prop.properties["set"] + " set), "
            while len(name) < 30:
                name += " "
            houses = "with " + (str(prop.houses) + " house" + ("s" if not prop.houses == 1 else "") if not prop.houses == 5 else "1 hotel") + ", "
            while len(houses) < 15:
                houses += " "
            rent = "which " + ("costs" if not prop.state == "mortgaged" else "would cost") + " $" + str(prop.properties["rent"][prop.houses])

            mortgage = ("" if not prop.state == "mortgaged" else ", but it is mortgaged")
            print name + houses + rent + mortgage
        clearSpace(1)
        return False
